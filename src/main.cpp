...
#if defined(NDEBUG)
# error "Lymiacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xaa9041a1f9ffa3235fd79bc4fbdf36f6268358b5b87573b96ef046c2796c2f63");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lymiacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lymiacoin Signed Message:\n";
...
{
    // Lymiacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Lymiacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Lymiacoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Lymiacoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lymiacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lymiacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1dba0fb1170ddc5ec70c9cc0a49f166aefa1f0675408e97dcdfa6e2720b67533");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Faut manger pour faire caca";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041dcf282bbd4a736058e29201a9577dab5f1fe5d4c9c8609922e92bb6b7a6f07dde49b8b6395d21821d2bea1eb33afa8ac06acf6bbc9db69eb65c1577211fb734") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1686464703;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085798378;

        if (fTestNet)
        {
            block.nTime    = 1686464688;
            block.nNonce   = 385363585;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd7971aa29330412bb46701acc13d14d53299365e22e1e4f324e85d31eb1322c3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfu, 0xc2, 0xb5, 0xd3 }; // Lymiacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LymiacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LymiacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LymiacoinMiner(CWallet *pwallet)
{
    printf("LymiacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lymiacoin-miner");
    ...
    try { loop {
        ...
        printf("Running LymiacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LymiacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LymiacoinMiner, pwallet));
}
